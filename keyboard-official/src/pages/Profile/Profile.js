    ...
    return (
        <div className="root">
            <Typography>
                ...
                <Title>介绍</Title>
                <Paragraph>
                大家好，我是小恐龙，也是一名机械键盘爱好者。我写这个网站的原因是之前在键盘群里和大家聊天发现目前还没有什么工具可以直观的显示当前键盘团购的进度，所以想了一下决定自己写一个，这样大家能一起用，也算为我们的键盘社区贡献一点自己的力量。这个网站的每一条数据都是我一个个找的，为了尽可能的保证数据准确性，我会找到对应的官方团购贴进行核对，但即使这样，<Text mark>由于我个人所能投入的精力有限，难免会出现数据不准确的情况，请大家谅解。如果大家发现数据不准确的情况请随时给我<a href={`mailto:${customerEmail}?subject=数据纠正`} >发邮件</a>，我会尽快的修正错误</Text>。同时如果大家对网站有什么建议也欢迎大家<a href={`mailto:${customerEmail}?subject=键盘网站建议`} >发邮件</a>给我，我会尽量改进。后续我也会开发更多的功能，希望这个网站能成为我们玩家群体共同的客制化网站，我会尽可能采纳大家的意见！
                </Paragraph>
                <Paragraph>
                <Text mark>如果你想要查询的套件或者键帽没有显示，欢迎<a href={`mailto:${customerEmail}?subject=数据添加`} >发邮件</a>提醒我，我会尽快添加</Text>。请在邮件中标明正确且完整的套件或键帽名称，如果能附带官方团购贴的链接或截图更好，这样可以帮助我大大节省确认信息的工作量，感谢！
                </Paragraph>
                <Paragraph>
                关于网站域名，在买之前我并不知道有mykeyboard.eu这个网站，俗话说买的早不如买的巧，头脑一热一口气买了10年...由于域名和云服务的费用我均为自费，所以暂时就不更换了，如果mykeyboard.eu的所有者有所介意的话可以发邮件联系我，到时我会对域名进行更换，谢谢理解。
                </Paragraph>
                ...
                <Paragraph>
                一个产品的进度是从截团那一天开始计算的，由于团购的具体发货时间即使是开团的作者本人也很难确定，所以网站中关于团购发货的默认值是优先取官方团购贴中预计发货时间的最晚时间计算。比如预计7月发货则默认为7月的最后一天发货。Q3发货则默认为9月的最后一天发货。预计6月或7月发货则默认为7月最后一天发货。这样计算的原因是想尽可能降低大家对于延期产生的负面情绪。另外，由于很多作者在团购贴中并未注明预计发货时间，所以我只能在不同的vendors里找，而<Text mark>不同的vendors在对同一个团购的截团时间和预计发货时间可能会出现不统一的情况</Text>，所以网站中的信息有可能与你所掌握的有些许出入，但总体来看对进度的影响不算大。
                </Paragraph>
